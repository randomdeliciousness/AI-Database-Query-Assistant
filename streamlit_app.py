...
def main():
    ...
    if 'assistant' not in st.session_state or st.session_state.get('use_mcp') != use_mcp:
        ...
        st.session_state.assistant = EnhancedQueryAssistant("chinook.db", openai_key, use_mcp=use_mcp)
    ...
    if query:
        try:
            ...
            if similar:
                ...
                for sq in similar:
                    st.write(f"• {sq['natural_query']} (latency: {sq.get('latency_ms', 'N/A')} ms)")
        except Exception as e:
            st.error(f"❌ Error: {str(e)}")

    with st.expander("ℹ️ About the Database"):
        st.write("Chinook sample database (digital media store). Ask anything about sales, artists, customers, etc.")
